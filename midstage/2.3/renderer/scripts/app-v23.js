...
function switchPage(pageId) {
  ...
  if (bridge?.switchPage) {
    return bridge.switchPage(pageId, { state });
  }
...
function renderDetailPage() {
  ...
  if (selected && detailPlanAdded && planLineQty != null && state.desiredQty !== planLineQty) {
...
function renderBulkPage() {
  ...
  if (bulkUiBridge?.renderBulkPage) {
    const bulkRowsWithMeta = (state.bulkRows || []).map((row) => {
